...
class CATH(data.Dataset):
    def __init__(self, path='./',  mode='train', max_length=500, test_name='L100', data = None):
        self.path = path
        self.mode = mode
    ...
    def cache_data(self):
        ...
        if not os.path.exists(self.path):
            ...
            raise Exception("no such file:{} !!!".format(self.path))
...
if __name__ == '__main__':
    cath_dataset = CATH(path='../data/CATH', mode='train', max_length=500, test_name='All')
    #data是一个字典
    data = cath_dataset.cache_data
    print(data)
