...
from methods.OSRAattention import OSRAttention
...
class Context(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, node_context=False,
                 edge_context=False):
...
class GeneralGNN(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, node_net='AttMLP', edge_net='EdgeMLP',
                 node_context=0, edge_context=0, ):
        ...
        self.attention2 = OSRAttention(dim=128)
        #self.edge_update2=DualEGraph(num_hidden,num_in,dropout=0.1,scale=30)
    ...
    def forward(self, h_V, h_E, edge_idx, batch_id):
        src_idx = edge_idx[0]
        dst_idx = edge_idx[1]
        self.attention2 = self.attention2.to('cuda:0')
        h_V2 = self.attention2(h_V)  # 注意力2
        ...
        #h_E2 = self.edge_update2(h_V, h_E, edge_idx, batch_id)
        #h_E= h_E +h_E2
...
class StructureEncoder(nn.Module):
    def __init__(self, hidden_dim, num_encoder_layers=3, dropout=0, node_net='AttMLP', edge_net='EdgeMLP',
                 node_context=True, edge_context=False):
        ...
        for i in range(num_encoder_layers):
            encoder_layers.append(
                module(hidden_dim, hidden_dim * 2, dropout=dropout, node_net=node_net, edge_net=edge_net,
                       node_context=node_context, edge_context=edge_context),
            )
